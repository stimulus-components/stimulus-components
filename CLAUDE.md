...
Prettier: no semicolons, print width 120 (`.prettierrc`). ESLint uses flat config (`eslint.config.mjs`, ESLint 10) built on `@nuxt/eslint-config`; there is no `.eslintrc`/`.eslintignore`. TypeScript runs in `strict` mode (root `tsconfig.json`) — type timer fields as `ReturnType<typeof setTimeout>` rather than `number`, and prefer real types over `// @ts-expect-error`/`any`. Tests use Vitest in jsdom — `Application.start()`, register the controller by its kebab name, then set `document.body.innerHTML` with the right `data-controller`/targets/values (see `components/character-counter/spec/index.test.ts`).
